document.addEventListener("DOMContentLoaded", function() {
    ...
    const partidosPorJornada = {
        ...
        5: ["Querétaro vs Cruz Azul", "Mazatlán vs Pachuca", "Tijuana vs Rayados", "Necaxa vs Juárez FC", "León vs Santos", "Atlas vs Pumas", "Tigres vs Chivas", "Toluca vs Atlético de San Luis"],
    ...
    function actualizarTabla(partidos) {
        ...
        tr.innerHTML = "<th>Eliminar</th><th>Nombre</th>";
    ...
    function eliminarUsuario(userId) {
        fetch(`/api/quiniela/usuario/${userId}`, {
            method: 'DELETE'
        })
        .then(response => response.json())
        .then(data => {
            alert(data.message);
            cargarQuinielasButton.click(); // Recargar quinielas
        })
        .catch(error => {
            console.error('Error:', error);
        });
